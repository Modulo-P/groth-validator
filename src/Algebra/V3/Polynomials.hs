...
module Algebra.V3.Polynomials where

import           BLS6_6.Params       (k0, poly0)

import           Data.Aeson          (FromJSON, ToJSON)

import           Algebra.V3.Integers (inverseI, (\*))
import           Algebra.V3.Types    (EuclideanRing (..), Field (..))
...
import           PlutusTx.Prelude    (AdditiveGroup (..), AdditiveMonoid (..),
                                      ...
                                      Semigroup (..), head, length, map,
                                      otherwise, snd, take, zipWith, (!!), ($),
                                      (&&), (.), (/=), (<$>), (<=))
import           Prelude             (Show)
...
instance MultiplicativeSemigroup Poly where
  {-# INLINABLE (*) #-}
  (*) :: Poly -> Poly -> Poly
  (*) (Poly !ps) (Poly !qs) = Poly $ mul ps qs
    where
      mul [] _             = [0]
      mul ((!x):(!xs)) !ys = zipWith (+) (map (x *) ys) (0 : mul xs ys)

testMul :: Poly
testMul = Poly [1, 2, 3] * Poly [1, 2, 3]
...
{-# INLINABLE degree #-}
degree :: Poly -> Integer
degree (Poly !ps) = go 0 0 ps
  where
      go l p (x:xs) = if mod0 x /= 0 then go p (p + 1) xs else go l (p + 1) xs
      go l _ []     = l
...
euclidesP (Poly !x) (Poly !y) = (Poly $ (i0 \*) <$> rs, Poly $ (i0 \*) <$> ss, Poly $ (i0 \*) <$> ts)
  where
    !i0 = inverseI . head $ rs
    oneArr = [1] <> replicate k0 0
    zeroArr = replicate (k0 + 1) 0
    (rs, ss, ts) = go (x, oneArr, zeroArr) (y, zeroArr, oneArr)
    go :: ([Integer], [Integer], [Integer]) -> ([Integer], [Integer], [Integer]) -> ([Integer], [Integer], [Integer])
    go (r0, s0, t0) (r1, s1, t1)
      | Poly r1 == zero = (r0, s0, t0)
      | otherwise  = let
            (Poly q, Poly r2) = divModE (Poly r0) (Poly r1)
            Poly s2      = Poly s0 - Poly q * Poly s1
            Poly t2      = Poly t0 - Poly q * Poly t1
          in
